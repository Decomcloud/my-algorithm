...
public class num44WildcardMatching {
    public static void main(String[] args) {
        Solution solution = new num44WildcardMatching().new Solution();
    }
    ...
    class Solution {
        ...
        private boolean dfs(String source, int sIndex, String pattern, int pIndex) {
            ...
            // pattern为*, 可能匹配多个, newSIndex代表字符串匹配后的index, *可以为空, 所以要包括当前index
            for (int newSIndex = sIndex; newSIndex <= source.length(); newSIndex++) {
                if (dfs(source, newSIndex, pattern, pIndex + 1)) {
                    return true;
                }
            }
            return false;
